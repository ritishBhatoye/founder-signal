...
export default function StripeConnectScreen() {
  ...
  const { connectStripeAccount, isConnecting, isConnected } = useStripeAccount(user?.id)
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      if (result.success && result.url) {
        ...
        Alert.alert('Connection Failed', result.error || 'Failed to connect to Stripe')
        setIsProcessing(false)
      }
    } catch (error) {
  ...
  return (
    <SafeAreaView className="flex-1" style={{ backgroundColor: colors.bg }}>
      <ScrollView className="flex-1" contentContainerStyle={{ padding: 16 }}>
        ...
        >
          <Text style={{ color: colors.text }} className="mb-4 text-lg font-semibold">
            What we&apos;ll access:
          </Text>
          ...
          <View className="flex-row items-start">
            ...
            <View className="ml-3 flex-1">
              ...
              <Text style={{ color: colors.text }} className="mt-1 text-sm">
                We use Stripe&apos;s official OAuth. Your credentials never touch our
                servers.
              </Text>
            </View>
          </View>
