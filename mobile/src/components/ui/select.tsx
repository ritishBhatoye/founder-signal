...
export const SelectInput = React.forwardRef<React.ElementRef<typeof Text>, SelectInputProps>(
...
export const SelectIcon: React.FC<{ as: () => React.ReactNode }> = ({ as }) => {
  return <>{as()}</>;
};
...
export const SelectBackdrop: React.FC<PressableProps & { className?: string }> = ({
  ...
  return (
    <Pressable className={`flex-1 ${className}`} onPress={() => setIsOpen(false)} {...props} />
  );
};
...
export const SelectDragIndicatorWrapper: React.FC<ViewProps> = ({ children, ...props }) => {
  return (
...
export const SelectDragIndicator: React.FC<ViewProps> = (props) => {
  return <View className="h-1 w-12 rounded-full bg-neutral-300 dark:bg-neutral-600" {...props} />;
};
