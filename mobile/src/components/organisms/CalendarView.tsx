...
const CalendarView: React.FC<CalendarViewProps> = ({
  ...
  const eventColors = { leave: "bg-warning-500", holiday: "bg-error-500", wfh: "bg-info-500" };

  return (
    <View className={`bg-white dark:bg-neutral-800 rounded-2xl p-4 ${className}`}>
      ...
      <View className="flex-row flex-wrap">
        ...
        {Array.from({ length: daysInMonth }).map((_, i) => {
          ...
          return (
            <TouchableOpacity
              key={day}
              onPress={() => onDayPress?.(new Date(month.getFullYear(), month.getMonth(), day))}
            ...
            >
              ...
              >
                <Text
                  className={`text-sm ${
                    isToday ? "text-white font-bold" : "text-neutral-900 dark:text-neutral-100"
                  }`}
                >
                  {day}
                </Text>
              ...
              {eventType && (
                <View
                  className={`w-1.5 h-1.5 rounded-full ${eventColors[eventType]} absolute bottom-0`}
                />
              )}
            </TouchableOpacity>
          );
