...
const SettingsScreenSection: React.FC<SettingsScreenSectionProps> = ({
  ...
  return (
    <View className={`bg-white dark:bg-neutral-800 rounded-2xl ${className}`}>
      {title && (
        <Text className="text-sm font-semibold text-neutral-500 uppercase px-4 pt-4 pb-2">
          {title}
        </Text>
      )}
        ...
        return (
          <TouchableOpacity
            ...
            className={`flex-row items-center px-4 py-4 ${
              index < items.length - 1 ? "border-b border-neutral-100 dark:border-neutral-700" : ""
            }`}
          >
            {item.icon && (
              <View className="w-10 h-10 rounded-full bg-neutral-100 dark:bg-neutral-700 items-center justify-center mr-3">
                <Ionicons
                  name={item.icon}
                  size={20}
                  color={
            ...
            <View className="flex-1">
              <Text
                className={`text-base font-medium ${
                  item.danger ? "text-error-500" : "text-neutral-900 dark:text-neutral-100"
                }`}
              >
                {item.label}
              </Text>
