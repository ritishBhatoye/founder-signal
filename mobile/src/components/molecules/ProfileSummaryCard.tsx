      ...
      <View className="flex-row items-center">
        <View className="relative">
          ...
          {onEdit && (
            <TouchableOpacity
              onPress={onEdit}
              className="absolute -bottom-1 -right-1 bg-primary-500 rounded-full p-1"
            >
        ...
        <View className="flex-1 ml-4">
          <Text className="text-lg font-bold text-neutral-900 dark:text-neutral-100">{name}</Text>
          {role && <Text className="text-sm text-neutral-600 dark:text-neutral-400">{role}</Text>}
          {department && <Text className="text-xs text-neutral-500">{department}</Text>}
        </View>
      </View>
      {stats.length > 0 && (
        <View className="flex-row mt-4 pt-4 border-t border-neutral-200 dark:border-neutral-700">
          {stats.map((stat, index) => (
            ...
            >
              ...
              <Text className="text-xs text-neutral-500 mt-1">{stat.label}</Text>
            </View>
          ))}
        </View>
      )}
