...
export const metricsApi = createApi({
  ...
  endpoints: (builder) => ({
    getMetrics: builder.query<MetricsResponse, void>({
      queryFn: async () => {
        try {
          ...
          const [todayMetrics, weekMetrics, monthMetrics, subscriptions] = await Promise.all([
            supabase
              .from("daily_metrics")
              .select("*")
              .eq("user_id", user.id)
          ...
          const calculateTotals = (data: DailyMetrics[]) => ({
            ...
            active_subscriptions: data.reduce((sum, d) => sum + d.active_subscriptions, 0),
            new_subscriptions: data.reduce((sum, d) => sum + d.new_subscriptions, 0),
            churned_subscriptions: data.reduce((sum, d) => sum + d.churned_subscriptions, 0),
            churn_rate: data.length > 0 
    ...
    getDailyMetrics: builder.query<DailyMetrics[], { startDate: string; endDate: string }>({
    ...
    getSupportPressure: builder.query<{ support_pressure: number }, void>({
      queryFn: async () => {
        try {
          ...
          return {
            error: {
              message: error instanceof Error ? error.message : "Failed to fetch support pressure",
            },
          };
        }
      },
    ...
    updateSupportPressure: builder.mutation<void, { support_pressure: number }>({
      queryFn: async ({ support_pressure }) => {
        try {
          ...
          return {
            error: {
              message: error instanceof Error ? error.message : "Failed to update support pressure",
            },
          };
        }
      },
